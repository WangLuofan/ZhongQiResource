...
@interface ZQDistrictRscRecommendTableViewCell () {
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if(self){
        ...
        self.titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(kControlMargin, kControlMargin, self.bounds.size.width - 3*kControlMargin, (self.bounds.size.height - 3*kControlMargin) / 2)];
...
-(void)setSelected:(BOOL)selected animated:(BOOL)animated {
    ...
    if(selected)
        [whiteShadowView setBackgroundColor:[UIColor grayColor]];
    else
        [whiteShadowView setBackgroundColor:[UIColor whiteColor]];
    return ;
}
